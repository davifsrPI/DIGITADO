...
const initialState: EntityState<ISala> = {
  loading: false,
  errorMessage: null,
  entities: [],
  entity: defaultValue,
...
export const getEntities = createAsyncThunk(
  'sala/fetch_entity_list',
  async ({ sort }: IQueryParams) => {
    const requestUrl = `${apiUrl}?${sort ? `sort=${sort}&` : ''}cacheBuster=${new Date().getTime()}`;
    return axios.get<ISala[]>(requestUrl);
  },
...
export const createEntity = createAsyncThunk(
  ...
  async (entity: ISala, thunkAPI) => {
    const result = await axios.post<ISala>(apiUrl, cleanEntity(entity));
...
export const deleteEntity = createAsyncThunk(
  ...
  async (id: string | number, thunkAPI) => {
    const requestUrl = `${apiUrl}/${id}`;
    const result = await axios.delete<ISala>(requestUrl);
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError },
...
export const SalaSlice = createEntitySlice({
  name: 'sala',
  initialState,
...
export const { reset } = SalaSlice.actions;
