...
import { getEntities } from './sala.reducer';
...
export const Sala = () => {
  ...
  return (
    <div>
      <h2 id="sala-heading" data-cy="SalaHeading">
        <Translate contentKey="digitadoApp.sala.home.title">Salas</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="digitadoApp.sala.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/sala/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="digitadoApp.sala.home.createLabel">Create new Sala</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {salaList && salaList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="digitadoApp.sala.professor">Professor</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="digitadoApp.sala.alunos">Alunos</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {salaList.map((sala, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {sala.alunos
                      ? sala.alunos.map((val, j) => (
                          <span key={j}>
                            <Link to={`/usuario/${val.id}`}>{val.id}</Link>
                            {j === sala.alunos.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="digitadoApp.sala.home.notFound">No Salas found</Translate>
            </div>
          )
        )}
      </div>
